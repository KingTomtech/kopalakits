...
function MenuDrawer({ isOpen, onClose }) {
  ...
  const menuItems = [
    ...
    { id: 'football', label: 'Zambian Football', icon: Trophy },
    { id: 'copperbelt', label: 'The Copperbelt', icon: MapPin },
        ...
        <div className="p-2">
          {menuItems.map(item => (
            <button
              ...
              style={{ 
                backgroundColor: page === item.id ? 'var(--olive-500)' : 'transparent',
                color: page === item.id ? 'var(--champagne-900)' : 'var(--olive-200)'
              }}
            >
              <item.icon size={18} />
...
function NewsWidget() {
  ...
  const { setPage } = React.useContext(NavContext);
  const latestNews = footballNews[0];
        ...
        >
          ...
          <div className="p-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <button onClick={goToNews} className="text-xs font-bold flex items-center gap-1" style={{ color: 'var(--olive-500)' }}>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      <section className="mb-8 p-6 rounded-2xl border" style={{ backgroundColor: 'var(--champagne-800)', borderColor: 'var(--khaki-700)' }}>
        ...
        <p className="mb-4" style={{ color: 'var(--olive-200)' }}>
          On February 12, 2012, Zambia achieved the ultimate glory by winning the Africa Cup of Nations 
          in Libreville, Gabon. The Chipolopolo defeated Ivory Coast on penalties, with goalkeeper Kennedy 
          Mweene and captain Christopher Katongo leading the team to their first-ever continental title.
        </p>
        <p style={{ color: 'var(--olive-400)' }}>
          The victory was especially emotional as it came near the site of the 1993 plane crash that 
          killed 18 members of the national team. The 2012 squad dedicated their triumph to their 
          fallen heroes.
        </p>
      </section>
      ...
      <section className="mb-8">
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: 'var(--olive-300)' }}>
          <Users size={20} style={{ color: 'var(--olive-500)' }} /> Top Zambian Clubs
        </h2>
        <div className="grid gap-4 md:grid-cols-2">
          {zambianTeams.map(team => (
            ...
            >
              ...
              <p className="text-sm" style={{ color: 'var(--olive-400)' }}>
                Founded: {team.founded} | {team.city}
              </p>
              <p className="text-sm mt-1" style={{ color: 'var(--olive-500)' }}>{team.achievements}</p>
              <p className="text-xs mt-2" style={{ color: 'var(--khaki-400)' }}>{team.stadium}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="p-6 rounded-2xl border" style={{ backgroundColor: 'var(--champagne-800)', borderColor: 'var(--khaki-700)' }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          {['Kalusha Bwalya', 'Christopher Katongo', 'Godfrey Chitalu', 'Kennedy Mweene'].map(legend => (
            <div key={legend} className="p-3 rounded-lg" style={{ backgroundColor: 'var(--khaki-700)' }}>
              <p className="font-bold text-sm" style={{ color: 'var(--olive-200)' }}>{legend}</p>
            </div>
          ))}
        </div>
      </section>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <header className="text-center py-8">
        ...
        <p style={{ color: 'var(--olive-400)' }}>{"Zambia's mining heartland and football powerhouse"}</p>
      </header>

      <section className="mb-8 p-6 rounded-2xl border" style={{ backgroundColor: 'var(--champagne-800)', borderColor: 'var(--khaki-700)' }}>
        <h2 className="text-xl font-bold mb-4" style={{ color: 'var(--olive-300)' }}>About the Region</h2>
        <p style={{ color: 'var(--olive-200)' }}>
          The Copperbelt Province is the industrial and mining heartland of Zambia, located in the 
          north-central part of the country. Home to over 2 million people, it is the second most 
          populous province and the most urbanized region in Zambia. The province is named after 
          the copper deposits that have been mined here since the 1920s.
        </p>
      </section>

      <section>
        <h2 className="text-xl font-bold mb-4 flex items-center gap-2" style={{ color: 'var(--olive-300)' }}>
          <MapPin size={20} style={{ color: 'var(--olive-500)' }} /> Major Towns
        </h2>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {copperbeltTowns.map(town => (
            ...
            >
              ...
              <p className="text-sm" style={{ color: 'var(--olive-400)' }}>Population: {town.population}</p>
              <p className="text-xs mt-2" style={{ color: 'var(--khaki-400)' }}>{town.known}</p>
            </div>
          ))}
        </div>
      </section>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <header className="text-center py-8">
        ...
        <p style={{ color: 'var(--olive-400)' }}>Latest updates from Zambian football</p>
      </header>
        ...
        {footballNews.map(news => (
          ...
          >
            <div className="flex justify-between items-start mb-2">
              <span className="text-xs font-bold px-2 py-1 rounded" style={{ backgroundColor: 'var(--olive-500)', color: 'var(--champagne-900)' }}>
                {news.source}
              </span>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <header className="text-center py-8">
        ...
        <p style={{ color: 'var(--olive-400)' }}>Your trusted source for quality football jerseys</p>
      </header>

      <section className="mb-8 p-6 rounded-2xl border" style={{ backgroundColor: 'var(--champagne-800)', borderColor: 'var(--khaki-700)' }}>
        <h2 className="text-xl font-bold mb-4" style={{ color: 'var(--olive-300)' }}>Our Story</h2>
        <p className="mb-4" style={{ color: 'var(--olive-200)' }}>
          Kopala Kits was founded in <strong>Luanshya</strong>, the heart of the Copperbelt mining region. 
          What started as a small operation has grown into the region&apos;s most trusted source for 
          authentic football jerseys.
        </p>
        <p style={{ color: 'var(--olive-200)' }}>
          Today, we operate from <strong>CBU K-Block in Kitwe</strong>, serving football fans across 
          the Copperbelt and beyond. We specialize in both local Zambian teams and international 
          club jerseys.
        </p>
      </section>

      <section className="mb-8 p-6 rounded-2xl border" style={{ backgroundColor: 'var(--champagne-800)', borderColor: 'var(--khaki-700)' }}>
        <h2 className="text-xl font-bold mb-4" style={{ color: 'var(--olive-300)' }}>Contact Us</h2>
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <MapPin size={20} style={{ color: 'var(--olive-500)' }} />
            <span style={{ color: 'var(--olive-200)' }}>CBU K-Block, Kitwe, Copperbelt Province</span>
          </div>
          <div className="flex items-center gap-3">
            <MessageCircle size={20} style={{ color: 'var(--olive-500)' }} />
            <span style={{ color: 'var(--olive-200)' }}>WhatsApp: +260 XXX XXX XXX</span>
          </div>
        </div>
      </section>

      <section className="text-center">
        <h2 className="text-xl font-bold mb-4" style={{ color: 'var(--olive-300)' }}>Follow Us</h2>
...
function Footer() {
  return (
    <footer className="p-8 text-center mt-8" style={{ backgroundColor: 'var(--champagne-800)' }}>
...
export default function KopalaKits() {
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'copperbelt': return <CopperbeltPage />;
